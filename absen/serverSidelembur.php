...
$sLimit = "";
	...
	if ( isset($input['sSearch']) && $input['sSearch'] != "" ) {
		...
		for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
			if ( isset($input['bSearchable_'.$i]) && $input['bSearchable_'.$i] == 'true' ) {
				...
				$aFilteringRules[] = "`".$aColumns[$i]."` LIKE '%".$mysqli->real_escape_string( $input['sSearch'] )."%' and absensi.id_abs > 0 and TIMESTAMPDIFF(HOUR,c.jam_pulang,jam_out) > 0";
			}
		}
	...
	for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
		if ( isset($input['bSearchable_'.$i]) && $input['bSearchable_'.$i] == 'true' && $input['sSearch_'.$i] != '' ) {
			$aFilteringRules[] = "`".$aColumns[$i]."` LIKE '%".$mysqli->real_escape_string($input['sSearch_'.$i])."%' and absensi.id_abs > 0 and TIMESTAMPDIFF(HOUR,c.jam_pulang,jam_out) > 0";
		}
	}
	...
	if (!empty($aFilteringRules)) {
		...
		$sWhere = "where absensi.id_abs > 0 and TIMESTAMPDIFF(HOUR,c.jam_pulang,jam_out) > 0";
	}
     ...
     */
    $sQuery = "
        SELECT SQL_CALC_FOUND_ROWS absensi.id_absensi,absensi.id_ijin,d.nm_ijin,absensi.nip,c.shift,jam_in,c.jam_masuk,jam_out,c.jam_pulang, TIMESTAMPDIFF(HOUR,c.jam_pulang,jam_out)as lembur_jam,TIMEDIFF(jam_out,c.jam_pulang)as lembur,status_masuk,status_keluar,terlambat,pulangcepat,ket,TIMEDIFF(jam_in,c.jam_masuk)as jam_telat, TIMEDIFF(c.jam_pulang,jam_out)as jam_p_cepat," . str_replace(" , ", " ", implode(", ", $aColumns)) . "
        FROM   $sTable
        $sJoin
        $sWhere
        $sOrder
