...
<tbody>
    ...
    $query = mysqli_query($mysqli,"select * from tabel_pesan where kepada='$nm_user' order by waktu");
    ...
    if(isset($_POST['cari'])) {
        ...
        $query = mysqli_query($mysqli,"select * from tabel_pesan where kepada='$nm_user' order by waktu desc LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");

    } else {
        $query = mysqli_query($mysqli,"select * from tabel_pesan where kepada='$nm_user' order by waktu desc LIMIT 0, $jml_per_halaman");
        $halaman = 1; //tambahan
    }
    ...
    <tr>
        ...
        <td>
           <a href="?id=msg&mod=del&nomor=<?php echo $data['nomor'] ?>" onclick="return confirm('Menghapus Pesan dari <?php echo $data['dari'] ?>')" name="hapus" class="hapus">
